...
model = TransformerModel(DOWNLOADED_MODEL_PATH)
model.to(config['device'])
if DATAPARALLEL:
    model=nn.DataParallel(model)
optimizer = torch.optim.Adam(params=model.parameters(), lr=config['learning_rates'][0], weight_decay=args.weight_decay)
