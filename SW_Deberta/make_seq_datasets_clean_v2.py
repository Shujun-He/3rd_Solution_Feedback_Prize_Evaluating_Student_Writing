...
for f in list(os.listdir(f'{DATA_PATH}/test')):
    test_names.append(f.replace('.txt', ''))
    test_texts.append(open(f'{DATA_PATH}/test/' + f, 'r').read())
...
for f in tqdm(list(os.listdir(f'{DATA_PATH}/train'))):
    test_names.append(f.replace('.txt', ''))
    train_texts.append(open(f'{DATA_PATH}/train/' + f, 'r').read())
...
if not SUBMISSION:
    try:
        ...
        # with open( f"../test69/cache_fix_prob_or/valid_preds_fold{args.fold}.p", "rb" ) as validFile:
...
#     #assert len(valid_word_preds[i])==len(valid_word_preds2[i])
#     if len(valid_word_preds[i])<len(valid_word_preds2[i]):
#         for j in range(len(valid_word_preds[i])):
#             valid_word_preds[i][j]=valid_word_preds[i][j]*0.7+valid_word_preds2[i][j]*0.3
#     else:
